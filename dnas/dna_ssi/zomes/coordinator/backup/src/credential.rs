...
pub fn create_credential(credential: Credential) -> ExternResult<Record> {
  ...
  let record = get(credential_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Could not find the newly created Credential"))))?;
...
pub fn update_credential(input: UpdateCredentialInput) -> ExternResult<Record> {
  let updated_credential_hash = update_entry(input.original_action_hash, &input.updated_credential)?;

  let record = get(updated_credential_hash.clone(), GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest(String::from("Could not find the newly updated Credential"))))?;
